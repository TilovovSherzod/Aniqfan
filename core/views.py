...
from .models import Presentation, Guide, Video, Topic, Practical, Lab, Test, Question, Choice, TestAttempt, Answer
from .serializers import PresentationSerializer, GuideSerializer, VideoSerializer
from .forms import GuideForm, VideoForm, TopicForm, PracticalForm, LabForm
...
def home(request):
    ...
    context = {"title": "ANIQ QISHLOQ XO'JALIGI"}
...
def topics_list(request):
    qs = Topic.objects.filter(is_published=True).order_by('-created_at')
    return render(request, 'core/topics_list.html', {'topics': qs})
...
@login_required
def topic_download(request, pk):
    topic = get_object_or_404(Topic, pk=pk)
    if not topic.file:
        raise Http404("Fayl topilmadi")
    path = topic.file.path
    if not os.path.exists(path):
        raise Http404("Fayl topilmadi")
    return FileResponse(open(path, 'rb'), as_attachment=True, filename=os.path.basename(path))
...
def topic_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('core:topics_list')
    else:
        form = TopicForm()
...
def test_edit(request, pk):
    """Edit test settings (time limit in minutes, title, description, publish)."""
    ...
    return render(request, 'core/test_form.html', {'form': form, 'editing': True, 'test': test})


@teacher_required
def question_create(request, test_pk):
    """Create a Question (and Choices) for a given Test via frontend teacher UI."""
    ...
    return render(request, 'core/question_form.html', {'form': qform, 'formset': formset, 'test': test})


@teacher_required
def question_edit(request, pk):
    """Edit an existing Question and its Choices."""
    ...
    return render(request, 'core/question_form.html', {'form': qform, 'formset': formset, 'test': test, 'question': q})
